...
Define_Module(BleMeshMac);

const Protocol BleMeshMac::blemesh("blemesh", "Bluetooth Mesh", Protocol::Layer::UnspecifiedLayer);

void BleMeshMac::initialize(int stage)
{
...
void BleMeshMac::updateStatusIdle(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void BleMeshMac::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void BleMeshMac::updateStatusCCA(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_TIMER_CCA: {
...
void BleMeshMac::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
...
void BleMeshMac::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void BleMeshMac::manageMissingAck(t_mac_event    /*event*/, cMessage *    /*msg*/)
{
...
void BleMeshMac::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void BleMeshMac::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void BleMeshMac::updateMacState(t_mac_states newMacState)
{
 ...
 */
void BleMeshMac::fsmError(t_mac_event event, cMessage *msg)
{
...
void BleMeshMac::startTimer(t_mac_timer timer)
{
...
simtime_t BleMeshMac::scheduleBackoff()
{
 ...
 */
void BleMeshMac::handleLowerPacket(Packet *packet)
{
...
//                nbRxFrames++;
                ...
                if (ackMessage != nullptr)
                    delete ackMessage;
                auto csmaHeader = makeShared<BleMeshMacHeader>();
                ...
                ackMessage->addTag<PacketProtocolTag>()->setProtocol(&blemesh);
...
void BleMeshMac::decapsulate(Packet *packet)
{
